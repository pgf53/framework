...
bool clArgs(int argc, char **argv){
    ...
    if(argc < 3) {
		printf("FORMAT: log-to-tablog -l logFile [-t <list|elist|apache|wellness|uri>] -o salida [-c (response code filtering)] [-b (labeled uris)] [-q (split path/query)]\n");
    ...
    while (1){
        ...
        c = getopt_long(argc, argv, "l:t:o:cbq", long_options, &option_index);
