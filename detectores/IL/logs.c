...
#define APACHE_LOG_TOKENS 24 //Maximum number of tokens on Apache entries
    ...
    if (!strncmp(&tmp_str[map.uri][pos],"https://",8) ) { pos += 8; urilength -= 8;}     // Elimnate http:// to avoid conflicts with rules with :
	...
	while (!strstr(tmp_str[map.uri + offset +1],"HTTP/") && ((map.uri + offset +1) < n)) {
		
		// Additional URI field
		strcat(logEntry->URI," ");
		strcat(logEntry->URI, tmp_str[map.uri + offset +1]);
		offset++;
		logEntry->urilen += strlen(tmp_str[map.uri + offset +1])+1;
    ...
    if ((map.referer >= 0) && (map.referer + offset < n)) 
        if (tmp_str[map.referer + offset])
            if (!strcmp(tmp_str[map.referer + offset],"-")) {
				...
				while ( ((map.referer + offset) < n) && (tmp_str[map.referer + offset][strlen(tmp_str[map.referer + offset])-1] != '"') ) {
...
int parse_elist_logEntry(char * logLine, Apache_logEntry * logEntry, log_map map, int logLinelen){
	...
	if( strstr(tmp, "HTTP/1.0") != NULL )
        ...
        logEntry->Protocol = _1_1;
    else if( strstr(tmp, "HTTP/2") != NULL )
        logEntry->Protocol = _2;
	else {
        logEntry->Protocol = _VOID;
        p -=9;
