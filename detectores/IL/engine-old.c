...
bool utf8decode(char *utfstr) {
	...
	printf("BEFORE UTF8 [%s]\n",utfstr);
	if (strstr(utfstr,"%") != NULL) { // Potentially UTF8
		p = tmpc;
		while (( *p++ = decode_code_point(&utfstr) )) { printf("%c",*(p-1)); };
		strcpy(utfstr,tmpc);
	printf("DECODED UTF8 [%s]\n",tmpc);
		return(true);
	} else return(false);
...
/*                
				...
				*/
				// Alternative urldecode: place ascii character
