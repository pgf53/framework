...
** CHANGES:
**
**  v3.5: Allow space between : and value
**  v3.5: Corrected bug in num_rules_file
**  v3.5: Added a protection to avoid commented rules (snort alike)
**  v3.5: PUT method considered valid
**  v3.5: Bug fix: content negation is now processed
**  v3.5: Bug fix for multiple ; in content/pcre
...
bool parse_snortRule(const char * origRule){
    ...
    if (isRuleOK && is_HTTP_rule) { //Check if header is OK
...
#endif
        strncpy(rule_str, origRule, SNORT_RULE_MAX);
        tmpfield = strstr(rule_str,"(");            /* Pointer to start of options */
        if (!tmpfield) {                              // Error in options
        ...
        URI_rule * rule = init_rule();      /* Empty rule creation */
        ...
        printf("\tOptions: %s\n",opt_str);
#endif
        /* One by one option parsing - each option is classified and analyzed */
		/* Problems with fields containing literal ';' (mainly content and pcre) */
       
        tmpfield = strtok(opt_str,";");

        /* Field segmentation, selection of fields of interest */
        
        while ((tmpfield) && strcmp(tmpfield,")")) {
#ifdef DEBUG
        	printf("\t\tSegment [%s]\n",tmpfield);
#endif
        	while(tmpfield[0]==' ') tmpfield ++;    /* Clean spaces at the begining */
            if (!strncmp(tmpfield,"msg:",4)) {     /* Message */
            
                tmpvalue = tmpfield + strlen("msg:"); /* Jump to message content */
                
				while (tmpvalue[0] == ' ') tmpvalue++; /* Allow space after : */
                ...
                printf("\t\t\tExtracted reference: [%s]\n",rule->references[rule->num_ref]);
                    ...
                    rule->attack_type[length] = '\0';
#ifdef DEBUG
                printf("\t\t\tExtracted attack_type: [%s]\n",rule->attack_type);
#endif
            } else if (!strncmp(tmpfield,"pcre:",5)) {

				// Parse the field even if it cannot be stored 
				...
				while (tmpvalue[0] == ' ') tmpvalue++; /* Allow space after : */
				if (tmpvalue[0] == '"') tmpvalue++; /* Avoid initial quotation mark */
				else {
                        printf("[parse_snortRule]: ERROR - Error in pcre field for rule at line [%d] - No quotation\n", num_rules_file);
						exit(-1);  					
				}
                length = strlen(tmpvalue);
				...
				content[0] = '\0';
				contentlength = 0;
				while (tmpvalue[length-1] != '\"') {	// This is not the last piece
					strncat(content,tmpvalue,length);
                    strcat(content,";");
					contentlength += length+1;
                    content[contentlength]='\0';					
					tmpvalue=strtok(NULL,";");  		// Next piece
#ifdef DEBUG
        	printf("\t\tSegment added [%s] FULL [%s]\n",tmpvalue,content);
#endif                    
					if (!tmpvalue) {
                        printf("[parse_snortRule]: ERROR - Error in pcre field for rule at line [%d]\n", num_rules_file);
						exit(-1);                      
					}
					length = strlen(tmpvalue);
				};
...
#ifdef DEBUG
        	printf("\t\tSegment last [%s] FULL [%s]\n",tmpvalue,content);
#endif 
				// Last piece (should end in \")

				strncat(content,tmpvalue,length-1);
				contentlength += length-1; 
				content[contentlength] = '\0';
                
				// Check whether the expression can be stored
				
				if (rule->num_pcre >= MAX_PCRE) {
					printf("[parse_snortRule] WARNING: Line [%d] exceeds allowed number of PCRE fields\n",num_rules_file);
                    ...
                    if  ( content[0] == '!')   {  // It is a negated regular expression 
                        ...
                        rule->pcre[rule->num_pcre].negated = negated;
                    }
                ...
                if (tmpvalue[0] == '"') tmpvalue++; /* Avoid initial quotation mark  */
                        ...
                        printf("[parse_snortRule]: ERROR - Error in content field for rule at line [%d] - No quotation [%s]\n", num_rules_file,tmpvalue);
						exit(-1);  					
				}
                length = strlen(tmpvalue);
				...
				while (tmpvalue[length-1] != '\"') {	// This is not the last piece
					...
					tmpvalue=strtok(NULL,";");  		// Next piece				
                   if (!tmpvalue) {
                        printf("[parse_snortRule]: ERROR - Error in content field for rule at line [%d]\n", num_rules_file);
						exit(-1);                      
					}
					length = strlen(tmpvalue);
				};
				...
				if (rule->num_patt < MAX_PATTERNS) {
                    if ((contentlength > 0) && (contentlength < CONTENT_LENGTH)) {
                        
                        rule->URI_pattern[rule->num_patt].pattern_str = uchar_malloc(contentlength+1);
                        strncpy(rule->URI_pattern[rule->num_patt].pattern_str, content, contentlength);
                        rule->URI_pattern[rule->num_patt].pattern_str[contentlength] = '\0';

                        parse_snortContent(rule->URI_pattern[rule->num_patt].pattern_str);
                        rule->URI_pattern[rule->num_patt].nocase = false;
						rule->URI_pattern[rule->num_patt].negated = negated;
#ifdef DEBUG
                        printf("\t\t\tContent extracted: [%s]\n",rule->URI_pattern[rule->num_patt].pattern_str);
#endif
                        rule->num_patt++;
                    } else {
                        printf("[parse_snortrule]: WARNING - Error parsing content in line [%d] - Content too large\n",num_rules_file);
                    }   
                } else if (!cwarning) {                  
                ...
                if (rule->num_patt > 0) {
                    ...
                    if ( (!strstr(rule->URI_pattern[rule->num_patt-1].pattern_str, "GET") || !strstr(rule->URI_pattern[rule->num_patt-1].pattern_str, "POST") || 
						!strstr(rule->URI_pattern[rule->num_patt-1].pattern_str, "HEAD") || !strstr(rule->URI_pattern[rule->num_patt-1].pattern_str, "PROPFIND") || !strstr(rule->URI_pattern[rule->num_patt-1].pattern_str, "PUT") ) )  {
								rule->num_patt--;
