...
num_total_lineas_index = sys.argv[4]	#número total de líneas del fichero de index
...
if os.environ["LAUNCH_MODE"] == "multiple":
	...
	with open(file_uri) as f:
		for uri in f:
			...
			#Comprobamos que el fichero .index no ha terminado
			if line_number_index <= final_line_index:
				...
				if os.environ["LAUNCH_TYPE"] == "online-local" or os.environ["LAUNCH_TYPE"] == "online-remoto":
					...
					#print("URI DE ENTRADA: " + uri_in + "\n")
					#print("URI DE INDEX: " + uri_index + "\n")
					if uri_in == uri_index:
						inserta_attack(line_index, uri, num_uri)
						line_number_index += 1
					else:
